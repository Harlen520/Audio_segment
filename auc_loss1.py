...
def binary_auc_score(y_true, y_score, delta=0.7):
    """
    Binary auc score.
    y_true:[samples]
    y_score:[samples]
    ...
    mpr = neg_score.reshape((1, neg_n)) - pos_score.reshape((pos_n, 1))
    ...
    auc_score = F.relu(paddle.flatten(mpr) + delta)
...
class AucLoss(Layer):
    ...
    def forward(self, input, label):
        ...
        return self.loss_fnt(input, label)
    ...
    def auc_ovo_score(self, input, label):
        ...
        for ix, (ci, cj) in enumerate(combinations(label_unique, 2)):
            ...
            ij_auc_score = binary_auc_score(i_true, input[ij_mask][:, ci])
            ji_auc_score = binary_auc_score(j_true, input[ij_mask][:, cj])
    ...
    def auc_ovr_score(self, input, label):
        ...
        for ix, ci in enumerate(label_unique):
            ci_mask = label == ci
            ij_auc_score = binary_auc_score(ci_mask, input[:, ci])
    ...
    def auc_binary_score(self, input, label):
        ...
        auc_score = binary_auc_score(c1_mask, y_pros)
